---
titulo: El Pozo Surgente
nivel: principiante
resumen: >-
  Descubrirás cómo evolucionan los pozos petroleros y gasíferos desde el
  inicio de la producción hasta su declinación, incluyendo intervenciones,
  optimización y adaptación de instalaciones. Un curso práctico para
  comprender el ciclo completo de vida de un pozo surgente y su importancia en
  la operación eficiente de los yacimientos.
temario:
  - Reservorios
  - Características de Vaca Muerta
  - Surgencia natural
  - Índice de producción
  - Puentes de producción
  - Control de producción
  - Tratamientos químicos
  - Reparaciones y mantenimiento
duracion: A tu ritmo · online
precio:
  ars: 185000
  usd: 120
orden: 2
publicado: true
destacado: false
---

Entender el pozo es entender la operación: **cómo surge el fluido, cómo se
comporta el flujo vertical y qué rol cumplen el arbolito y el puente de
producción**. Este curso construye el modelo mental del pozo que después usan
todos los demás cursos de la academia.

Para operadores de campo que quieren pasar de manipular válvulas a entender
qué pasa del otro lado.
